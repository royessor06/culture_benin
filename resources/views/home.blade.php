...
<div class="max-w-6xl mx-auto py-12 px-4">
    ...
    <div class="grid md:grid-cols-3 gap-8">
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden transform transition duration-300 hover:-translate-y-2 hover:shadow-2xl animate-cardFade">
            <img src="{{ $contenu->image }}" class="w-full h-48 object-cover" alt="Image contenu" />

            <div class="p-5">
                <h3 class="text-xl font-bold mb-2">{{ $contenu->titre }}</h3>
                <p class="text-gray-700 line-clamp-3">{{ Str::limit($contenu->description, 120) }}</p>

                <div class="flex justify-between items-center mt-4">
                    <a href="{{ route('contenu.show', $contenu->id) }}" class="text-blue-600 hover:underline font-semibold">Voir plus</a>

                    <button class="p-2 rounded-full bg-gray-100 hover:bg-blue-600 hover:text-white transition" onclick="window.location='{{ route('contenu.show', $contenu->id) }}#comments'">
