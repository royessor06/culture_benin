...
@section('title', 'Dashboard Admin - Culture Bénin')
@section('page-title', 'Tableau de bord')
@section('page-subtitle', 'Aperçu des performances et activités')
            ...
            <div class="card-body">
                ...
                <div class="d-flex align-items-center">
                    <span class="badge bg-success bg-opacity-10 text-success fs-7 fw-semibold px-2 py-1 rounded">
                        <i class="fas fa-arrow-up"></i> {{ $stats['user_growth'] ?? 0 }}%
                    </span>
                    <span class="text-muted ms-2 small">ce mois</span>
                </div>
                <div class="progress mt-2" style="height: 4px;">
                    <div class="progress-bar bg-primary" style="width: {{ $stats['user_growth'] ?? 0 }}%"></div>
                </div>
            </div>
            <div class="card-footer bg-transparent border-top-0 pt-0">
                <a href="{{ route('administrateur.index') }}" class="text-decoration-none text-primary fw-semibold small d-flex align-items-center justify-content-between">
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start mb-3">
                    <div>
                        ...
                        <h2 class="fw-bold mb-0">{{ $stats['total_contents'] ?? 0 }}</h2>
                    </div>
                ...
                <div class="d-flex align-items-center">
                    <span class="badge bg-info bg-opacity-10 text-info fs-7 fw-semibold px-2 py-1 rounded">
                        {{ $stats['published_contents'] ?? 0 }} publiés
                    </span>
                    <span class="text-muted ms-2 small">{{ $stats['pending_contents'] ?? 0 }} en attente</span>
                </div>
                <div class="d-flex mt-2">
                    <div class="flex-fill me-1">
                        <small class="text-muted d-block">Publiés</small>
                        <div class="progress" style="height: 4px;">
                            <div class="progress-bar bg-success" style="width: {{ $stats['published_percentage'] ?? 0 }}%"></div>
                        </div>
                    </div>
                    <div class="flex-fill ms-1">
                        <small class="text-muted d-block">En attente</small>
                        <div class="progress" style="height: 4px;">
                            <div class="progress-bar bg-warning" style="width: {{ $stats['pending_percentage'] ?? 0 }}%"></div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="card-footer bg-transparent border-top-0 pt-0">
                <a href="{{ route('contenu.index') }}" class="text-decoration-none text-primary fw-semibold small d-flex align-items-center justify-content-between">
            ...
            <div class="card-body">
                ...
                <div class="d-flex align-items-center">
                    <span class="badge bg-warning bg-opacity-10 text-warning fs-7 fw-semibold px-2 py-1 rounded">
                        <i class="fas fa-clock"></i> {{ $stats['pending_comments'] ?? 0 }} à modérer
                    </span>
                </div>
                <div class="mt-2">
                    <small class="text-muted">Dernier: {{ $stats['last_comment_time'] ?? 'Aucun' }}</small>
                </div>
            </div>
            <div class="card-footer bg-transparent border-top-0 pt-0">
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start mb-3">
                    <div>
                        ...
                        <h2 class="fw-bold mb-0">{{ number_format($stats['total_revenue'] ?? 0, 0, ',', ' ') }} FCFA</h2>
                    </div>
                ...
                <div class="progress mt-2" style="height: 4px;">
                    <div class="progress-bar bg-warning" style="width: {{ min($stats['revenue_growth'] ?? 0, 100) }}%"></div>
                </div>
            </div>
            <div class="card-footer bg-transparent border-top-0 pt-0">
                <a href="#" class="text-decoration-none text-primary fw-semibold small d-flex align-items-center justify-content-between">
                        ...
                        <div class="d-flex align-items-center">
                            <div class="flex-shrink-0">
                                <div class="avatar-sm rounded-circle bg-light d-flex align-items-center justify-content-center">
                                    <i class="fas fa-{{ $activity['icon'] ?? 'bell' }} text-{{ $activity['color'] ?? 'primary' }}"></i>
                                </div>
                            </div>
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-3 col-6">
                        <a href="{{ route('contenu.create') }}" class="card action-card text-decoration-none">
                    ...
                    <div class="col-md-3 col-6">
                        <a href="{{ route('moderateur.index') }}" class="card action-card text-decoration-none">
                            <div class="card-body text-center p-4">
                                <div class="action-icon mb-3">
                    ...
                    <div class="col-md-3 col-6">
                        <a href="{{ route('langue.index') }}" class="card action-card text-decoration-none">
                            <div class="card-body text-center p-4">
                                <div class="action-icon mb-3">
                    ...
                    <div class="col-md-3 col-6">
                        <a href="{{ route('region.index') }}" class="card action-card text-decoration-none">
                            <div class="card-body text-center p-4">
                                <div class="action-icon mb-3">
    ...
    <div class="col-12">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-transparent border-0 d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('contenu.index') }}" class="btn btn-outline-primary btn-sm">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            @forelse($recent_contents ?? [] as $content)
                            <tr>
                                <td>
                                    <div class="d-flex align-items-center">
                                        @if($content->medias->count() > 0)
                                        <img src="{{ asset('storage/' . $content->medias->first()->chemin) }}"
                                             alt="{{ $content->titre }}"
                                             class="rounded me-2" style="width: 40px; height: 40px; object-fit: cover;">
                                        @endif
                                        <div>
                                            <h6 class="mb-0">{{ Str::limit($content->titre, 40) }}</h6>
                                            <small class="text-muted">{{ Str::limit($content->texte, 50) }}</small>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <span class="badge bg-light text-dark">{{ $content->typeContenu->nom ?? 'Non défini' }}</span>
                                </td>
                                <td>{{ $content->auteur->prenom ?? '' }} {{ $content->auteur->nom ?? '' }}</td>
                                <td>{{ $content->created_at->format('d/m/Y') }}</td>
                                <td>
                                    <span class="badge bg-{{ $content->statut === 'Publié' ? 'success' : ($content->statut === 'Brouillon' ? 'warning' : 'secondary') }}">
                                        {{ $content->statut }}
                                    </span>
                                </td>
                                <td class="text-end">
                                    <div class="btn-group btn-group-sm">
                                        <a href="{{ route('contenu.show', $content->id) }}" class="btn btn-outline-primary">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('contenu.edit', $content->id) }}" class="btn btn-outline-warning">
