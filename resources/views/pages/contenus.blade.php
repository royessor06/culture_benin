...
<section class="detail-page" style="padding: 60px 8%;">
    ...
    <div class="section-title">
        <h2>{{ ucfirst($type->nom) }} du Bénin</h2>
    </div>
    ...
    <article class="contenu-item" style="background:white; padding:25px; border-radius:15px; box-shadow:0 3px 12px rgba(0,0,0,0.1); margin-bottom:40px; position:relative;">
        ...
        <div class="contenu-preview" id="preview-{{ $contenu->id }}">
            ...
            <div class="media-preview" style="margin-top:15px;">
                @if($contenu->medias->count() > 0)
                    ...
                    <img src="{{ asset('storage/' . $firstMedia->chemin) }}"
                         ...
                         onclick="openPopup({{ $contenu->id }})">
                    @if($contenu->medias->count() > 1)
                        <p style="font-size:0.9rem; color:#666;">
                            + {{ $contenu->medias->count() - 1 }} autre(s) image(s)
                        </p>
                    @endif
                @endif
            </div>
            ...
            <!-- Bouton Voir Plus -->
            @if(strlen($contenu->texte) > 200 || $contenu->medias->count() > 1)
            <div style="margin-top:15px;">
                <button class="voir-plus-btn" onclick="openPopup({{ $contenu->id }})"
...
<script>
    ...
    const contenusData = {
        ...
        {{ $contenu->id }}: {
            id: {{ $contenu->id }},
            titre: "{{ addslashes($contenu->titre) }}",
            texte: `{!! str_replace('`', '\\`', $contenu->texte) !!}`,
            medias: [
                @foreach($contenu->medias as $media)
                {
                    id: {{ $media->id }},
                    chemin: "{{ asset('storage/' . $media->chemin) }}",
                    type: "{{ $media->type }}"
                },
                @endforeach
            ],
            auteur: "{{ $contenu->auteur->prenom ?? 'Anonyme' }} {{ $contenu->auteur->nom ?? '' }}",
            date: "{{ $contenu->created_at->format('d/m/Y') }}",
            type: "{{ $contenu->type->nom ?? '' }}"
        },
        @endforeach
    };
